...
body{font-family:Arial,sans-serif;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;padding:20px}
h1{font-size:24px;margin-bottom:10px}
.status-bar{display:flex;gap:10px;margin-bottom:20px}
.status{padding:10px 20px;background:rgba(255,255,255,0.2);border-radius:20px;font-size:14px}
.status.connected{background:rgba(76,175,80,0.8)}
.status.disconnected{background:rgba(244,67,54,0.8)}
.joystick-container{position:relative;width:300px;height:300px;background:rgba(255,255,255,0.1);border:3px solid rgba(255,255,255,0.3);border-radius:50%;margin:20px;box-shadow:0 10px 40px rgba(0,0,0,0.3)}
.joystick-base{position:absolute;top:50%;left:50%;width:80%;height:80%;transform:translate(-50%,-50%);border:2px dashed rgba(255,255,255,0.3);border-radius:50%}
.joystick-stick{position:absolute;top:50%;left:50%;width:80px;height:80px;background:radial-gradient(circle,#fff,#ccc);border:3px solid #888;border-radius:50%;transform:translate(-50%,-50%);box-shadow:0 5px 20px rgba(0,0,0,0.5);cursor:pointer;transition:box-shadow 0.1s}
.joystick-stick.active{box-shadow:0 5px 30px rgba(255,255,255,0.6);border-color:#4CAF50}
.coordinates{margin-top:20px;font-size:16px;font-family:monospace}
.instructions{margin-top:20px;text-align:center;font-size:12px;opacity:0.8;max-width:300px}
.emergency-stop{margin-top:20px;padding:15px 40px;font-size:18px;font-weight:bold;background:#f44336;color:#fff;border:none;border-radius:10px;cursor:pointer;box-shadow:0 5px 15px rgba(0,0,0,0.3)}
...
if(data.bleStatus!==undefined){
if(data.bleStatus==='connected'){
...
const normalizedY=-finalY/maxRadius;
