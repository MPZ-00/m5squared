/**
 * M25 Protocol Handler
 * 
 * CRC-16 calculation, packet validation, and protocol utilities
 */

#ifndef M25_PROTOCOL_H
#define M25_PROTOCOL_H

#include <Arduino.h>

// M25 Protocol constants
...
/**
 * Calculate CRC-16 checksum (matching m25_protocol.py)
 */
 ...
 */
static void printPacket(uint8_t* data, size_t len) {
    Serial.println("\n--- Packet Contents ---");
    ...
    if (len >= 16) {
        ...
        Serial.printf("  [0] Device ID: 0x%02X\n", data[0]);
        Serial.printf("  [1] Sequence: 0x%02X\n", data[1]);
        Serial.printf("  [2] Command: 0x%02X\n", data[2]);
        Serial.printf("  [3] Length: 0x%02X\n", data[3]);
        Serial.printf("  [4] Sub-cmd: 0x%02X\n", data[4]);
        Serial.printf("  [5] Reserved: 0x%02X\n", data[5]);
        if (len >= 10) {
            Serial.printf("  [6-9] Data: 0x%02X 0x%02X 0x%02X 0x%02X\n", 
